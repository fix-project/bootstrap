...
int _Z17__wasilibc_dttoifi( int a )
{
  return __wasilibc_dttoif( a );
}

int _Z6setjmpPv( void* );
int _Z6setjmpPv( void* addr )
{
  return setjmp( addr );
}
void _Z7longjmpPvi( void*, int );
void _Z7longjmpPvi( void* addr, int val )
{
  longjmp( addr, val );
}

int posix_madvise( void*, int, int );
int posix_madvise( void* addr, int len, int advise )
{
  return 0;
}
